...
function startSimulation(numberOfElements) {
  ...
  for (let i = 0; i < NUMBER_OF_TEST_RUNS; i++) {
    ...
    for (let j = 0; j < ITERATIONS; j++) {
      // Access and delete a random element
      const randomIndex = Math.floor(Math.random() * numbers.length);
      numbers.splice(randomIndex, 1); // Remove 1 element at the randomIndex
    }
  ...
  console.log(`Standard JS Array - Accessing and deleting a random item 1000 times`);
