...
function startSimulation(numberOfElements) {
    ...
    const NUMBER_OF_AVERAGE_TEST_RUNS = 100;
    const ITERATIONS = 1000;
    let totalTimeTaken = 0;

    for (let i = 0; i < NUMBER_OF_AVERAGE_TEST_RUNS; i++) {
        ...
        for (let i = 0; i<ITERATIONS; i++){
            minHeap.removeMin(); 
            minHeap.insert(Math.floor(Math.random() * MAX_RANDOM_NUMBER_SIZE) + 1)
        }
    ...
    console.log(`MinHeap implementation - Adding, removing and sorting items 1000 times`);
